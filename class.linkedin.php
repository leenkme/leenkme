<?php
/**
 * Registers leenk.me LinkedIn class for setting up leenk.me
 *
 * @package leenk.me
 * @since 3.0.0
 */
...
if ( !class_exists( 'LeenkMe_LinkedIn' ) ) {
	...
	class LeenkMe_LinkedIn {
		...
		function leenkme_admin_menu() {
			...
			add_submenu_page( 'leenkme', __( 'LinkedIn', 'leenkme' ), __( 'LinkedIn', 'leenkme' ), apply_filters( 'leenkme_menu_access_capability', 'edit_posts' ), 'leenkme-linkedin', array( $this, 'settings_page' ) );
		...
		function get_settings() {
			...
			$defaults = apply_filters( 'leenkme_linkedin_default_settings', $defaults );
		
			$settings = get_option( 'leenkme_linkedin' );
			
			return wp_parse_args( $settings, $defaults );
		}
		
		function update_settings( $settings ) {
			update_option( 'leenkme_linkedin', $settings );
		}
